...
export function CreateBankAccountModal({
  ...
  useEffect(() => {
    ...
    const autoName = textPart ? (emojiBlock ? emojiBlock + " " : "") + textPart : emojiBlock || "";
    if (!autoName) return;
    const wasAutoGenerated = !name.trim() || name === lastAutoNameRef.current;
    const missingEmojis = emojiBlock && !name.startsWith(emojiBlock) && wasAutoGenerated;
    const canUpdate = wasAutoGenerated || missingEmojis;
    if (canUpdate) {
            ...
            >
              ...
              {accountTypes.map((t) => (
                <option key={t.id} value={t.id}>
                  {(t.emoji?.trim() ? t.emoji.trim() + " " : "") + t.name}
                </option>
              ))}
            </Select>
            ...
            >
              ...
              {accountStatuses.map((s) => (
                <option key={s.id} value={s.id}>
                  {(s.emoji?.trim() ? s.emoji.trim() + " " : "") + s.name}
                </option>
              ))}
            </Select>
