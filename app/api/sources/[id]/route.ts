...
export async function PATCH(
  ...
  try {
    ...
    if (name !== undefined && !name) {
      return NextResponse.json(
        { error: "Le nom de la source ne peut pas être vide." },
        { status: 400 }
      );
    }
    const [existing] = await sql`
      SELECT id, name, sort_order FROM sources WHERE id = ${id}::uuid
    `;
    if (!existing) {
      return NextResponse.json(
...
export async function DELETE(
  ...
  try {
    const rows = await sql`
      DELETE FROM sources WHERE id = ${id}::uuid RETURNING id
    `;
    if (rows.length === 0) {
      return NextResponse.json(
        { error: "Source introuvable." },
