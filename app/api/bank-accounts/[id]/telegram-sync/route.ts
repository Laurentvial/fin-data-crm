...
export async function POST(
  ...
  if (!hasWork) {
    return NextResponse.json(
      {
        error:
          "Aucune action sélectionnée : titre, logo, invitations, promotions ou message.",
      },
      { status: 400 },
    );
  }
  ...
  try {
    ...
    const usersPayload: { telegram_id: number; telegram_username?: string }[] =
      [];
    if (inviteTelegramIds.length > 0) {
      ...
      `;
      const byId = new Map(
        links.map((r) => [Number(r.telegram_id), r.telegram_username as string | null]),
      );
      for (const tid of inviteTelegramIds) {
        const uname = byId.get(tid);
        usersPayload.push({
          telegram_id: tid,
          ...(typeof uname === "string" && uname
    ...
    if (promoteOnlyIds.length > 0) {
      payload.promote_only_telegram_ids = promoteOnlyIds;
    }
